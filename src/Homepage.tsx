...
const Modal = ({ onClose, children }) => (
  <div
    onClick={onClose}
    style={{
      position: "fixed", inset: 0, background: "rgba(27,20,100,0.45)", zIndex: 9999,
    ...
    >
      <button
        ...
        style={{
          ...
          fontSize: 22, cursor: "pointer", color: "#1B1464", lineHeight: 1, fontWeight: 900,
        }}
      >✕</button>
      {children}
    </div>
...
const AboutContent = () => (
  <div>
    <h2 style={{ fontFamily: "'Boogaloo', cursive", fontSize: 44, color: "#FF69B4", letterSpacing: 1, marginBottom: 8, marginTop: 0 }}>
      About Us
    </h2>
    <p style={{ fontSize: 14, fontWeight: 700, color: "#1B1464", lineHeight: 1.8, marginBottom: 16 }}>
    ...
    <p style={{ fontSize: 14, fontWeight: 600, color: "#1B1464", lineHeight: 1.8, marginBottom: 20, opacity: 0.85 }}>
      Founded 6 years ago by a group of college students who wanted to make campus life more connected, we've grown into a platform trusted by 700+ students. Our mission has never changed: help every student find their tribe.
    </p>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, margin: "24px 0" }}>
      {[
        ...
        <div key={i} style={{ background: "#FFF0F7", border: "2px solid #1B1464", borderRadius: 14, padding: "18px 20px" }}>
          <i className={item.icon} style={{ color: "#FF69B4", fontSize: 20, marginBottom: 8, display: "block" }} />
    ...
    <p style={{ fontSize: 13, fontWeight: 600, color: "#1B1464", lineHeight: 1.8, opacity: 0.8, marginBottom: 0 }}>
      We're a small, passionate team working every day to make campus connections easier, louder, and more meaningful.
    </p>
  </div>
);
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
        {[
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, background: "#FFF0F7", border: "2px solid #1B1464", borderRadius: 99, padding: "8px 16px", fontSize: 12, fontWeight: 700, color: "#1B1464" }}>
      ...
      {sent ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button
            ...
            style={{ padding: "14px", background: "#FF69B4", border: "2.5px solid #1B1464", borderRadius: 99, fontWeight: 900, fontSize: 14, textTransform: "uppercase", letterSpacing: 1, color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'Nunito',sans-serif" }}
...
const FAQContent = () => {
  ...
  const faqs = [
    { q: "Is CampusCrew free to use?", a: "Yes! CampusCrew is completely free for all students. You can post events, explore opportunities, and connect with teammates at zero cost — always." },
    { q: "Who can post events?", a: "Any student, club, or campus organization can post events. Whether it's a hackathon, cultural fest, sports trial, or study group — if it's campus-related, it belongs here." },
    { q: "How do I get started?", a: "Simply click 'Post an Event', fill in your event details, and hit submit. Your event will be visible to the entire campus community within minutes." },
    { q: "Can I filter events by category?", a: "Absolutely! Head to the Explore page and use our filters to browse events by category, date, keyword, or popularity." },
    { q: "How do I connect with a team?", a: "Each event listing has contact info or a direct connect button. You can reach out to organizers or teammates directly through the platform." },
    { q: "Is my information safe?", a: "We take privacy seriously. Your personal information is never sold or shared. Only contact details you choose to display publicly are visible to other students." },
    { q: "Can I edit or delete my event after posting?", a: "Yes, you can edit or remove your event at any time from your dashboard after logging in." },
  ];
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 14, fontWeight: 600, color: "#1B1464", lineHeight: 1.8, marginBottom: 24, opacity: 0.85 }}>
        Got questions? We've answered the most common ones below.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {faqs.map((faq, i) => (
          <div key={i} style={{ border: "2px solid #1B1464", borderRadius: 14, overflow: "hidden", background: open === i ? "#FFF0F7" : "#fff" }}>
            <button
              onClick={() => setOpen(open === i ? null : i)}
              style={{ width: "100%", background: "none", border: "none", padding: "16px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", fontFamily: "'Nunito', sans-serif", fontSize: 14, fontWeight: 800, color: "#1B1464", textAlign: "left", gap: 12 }}
            >
              <span>{faq.q}</span>
              <i className={open === i ? "fa-solid fa-chevron-up" : "fa-solid fa-chevron-down"} style={{ color: "#FF69B4", fontSize: 13, flexShrink: 0 }} />
            </button>
            {open === i && (
...
const NavLink = ({ onClick, children }) => {
  ...
  return (
    <button
      ...
      style={{ background: "none", border: "none", fontFamily: "'Nunito',sans-serif", fontWeight: 700, fontSize: 14, color: hov ? "#FF69B4" : "#1B1464", cursor: "pointer", transition: "color 0.18s", padding: "4px 2px" }}
    >
      {children}
    </button>
  );
...
export default function HomePage() {
  ...
  const [modal, setModal] = useState(null); // "about" | "contacts" | "faq" | null

  const stats = [
    { num: "700+", label: "Students",  desc: "actively finding their campus crew", icon: "fa-solid fa-user-group" },
    { num: "6",    label: "Years",     desc: "connecting campus communities",       badge: "Loved By Students" },
    { num: "100%", label: "Events",    desc: "open and free for all students",      badge2: "Campus First" },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Nunito',sans-serif", background: C.white, color: C.navy }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", borderBottom: C.border, height: 64, background: C.white, position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ fontFamily: "'Boogaloo',cursive", fontSize: 22, color: C.navy, letterSpacing: 1 }}>CampusCrew</div>
        ...
        <button
          style={{ padding: "8px 20px", background: C.pink, border: C.border, borderRadius: 99, fontWeight: 800, fontSize: 13, color: C.white, cursor: "pointer", display: "flex", alignItems: "center", gap: 7, fontFamily: "'Nunito',sans-serif" }}
      ...
      {/* ── HERO ── */}
      <section style={{ padding: "0 48px", position: "relative", borderBottom: C.border, overflow: "hidden", minHeight: 320, display: "flex", alignItems: "center", background: C.white }}>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr 200px", gap: 48, padding: "52px 48px", borderBottom: C.border, alignItems: "center" }}>
        <div style={{ fontSize: 14, lineHeight: 1.8, color: C.navy, fontWeight: 600 }}>
          ...
          is to help every student find their tribe — the events, clubs, and teammates that make college life extraordinary.
        </div>
        <div style={{ fontSize: 14, lineHeight: 1.8, color: C.navy, fontWeight: 600 }}>
          Whether you're a fresher trying to find someone to team up for your first hackathon or an art enthusiast waiting for their partner, CampusCrew is your trusted partner.
        </div>
          ...
          {features.map((f, i) => {
            ...
            return (
              <div
                ...
                onMouseEnter={() => setHov(true)}
                onMouseLeave={() => setHov(false)}
                style={{ background: C.pinkCard, border: C.border, borderRadius: 16, padding: "32px 24px", transition: "all 0.22s", transform: hov ? "translateY(-5px)" : "translateY(0)", boxShadow: hov ? `4px 8px 0 ${C.navy}` : "none" }}
              >
                <i className={f.icon} style={{ fontSize: 30, color: C.navy, marginBottom: 16, display: "block" }}/>
                <div style={{ fontFamily: "'Boogaloo',cursive", fontSize: 22, color: C.pink, marginBottom: 8 }}>{f.title}</div>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", background: C.pinkPale, borderTop: C.border }}>
        ...
        <div style={{ padding: "56px 48px", display: "flex", flexDirection: "column", gap: 16, justifyContent: "center" }}>
          {sent ? (
            ...
            <>
              {[["name","Your Name","text"],["phone","Phone Number","tel"],["email","E-mail","email"]].map(([key, ph, type]) => (
                <input
                  key={key}
                  type={type}
                  placeholder={ph}
                  value={form[key]}
                  onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  style={{ width: "100%", padding: "14px 20px", border: `2px solid ${C.pink}`, borderRadius: 99, background: "transparent", fontFamily: "'Nunito',sans-serif", fontSize: 14, fontWeight: 600, color: C.navy, outline: "none", boxSizing: "border-box" }}
              ...
              <button
                ...
                style={{ padding: "15px", background: btnHov ? C.navy : C.pink, border: C.border, borderRadius: 99, fontWeight: 900, fontSize: 14, textTransform: "uppercase", letterSpacing: 1, color: C.white, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'Nunito',sans-serif" }}
              >
      ...
      {/* ── FOOTER ── */}
      <footer style={{ borderTop: C.border, padding: "26px 48px", display: "flex", alignItems: "center", justifyContent: "space-between", background: C.white }}>
        ...
        <div style={{ display: "flex", gap: 28 }}>
          {[["About","about"],["Explore",null],["Contact","contacts"],["FAQ","faq"]].map(([label, target]) => {
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[["fa-brands fa-instagram","ig"],["fa-brands fa-facebook-f","fb"],["fa-brands fa-telegram","tg"]].map(([icon, id]) => {
            const [h, setH] = useState(false);
            return (
              <button key={id}
                onMouseEnter={() => setH(true)}
